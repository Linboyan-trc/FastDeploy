...
# 3.16 DeepGEMM
from fastdeploy.model_executor.layers.quantization.fp8_utils import per_token_group_quant_fp8
from fastdeploy.platforms import current_platform
if current_platform.is_cuda():
    from fastdeploy.model_executor.ops.gpu import (cp_gather_indexer_k_quant_cache, indexer_k_quant_and_cache, radix_topk_ragged_transform)
...
class DeepseekV3MLAAttention(nn.Layer):
    ...
    def __init__(self, fd_config: FDConfig, layer_id: int, prefix: str = "") -> None:
        ...
        self.qkv_a_proj_with_mqa = MergedReplicatedLinear(
            ...
            input_size=self.hidden_size, output_sizes=[self.q_lora_rank, self.kv_lora_rank + self.qk_rope_head_dim], with_bias=False,
        )
...
class DeepSeekV3DecoderLayer(nn.Layer):
    ...
    def __init__(
        ...
        if fd_config.model_config.model_type == "deepseek_v32":
            self.self_attn = DeepseekV32DSAAttention(   fd_config=fd_config,    layer_id=layer_id,  prefix=f"{prefix}.self_attn")
        else:
...
class DeepSeekV3Model(nn.Layer):
    ...
    def __init__(self, fd_config: FDConfig = None):
        ...
        # 1.2 输入层 + Transformer层 + 输出层归一化
        self.embed_tokens = VocabParallelEmbedding(
            fd_config, 
            num_embeddings=fd_config.model_config.vocab_size, embedding_dim=fd_config.model_config.hidden_size, 
            params_dtype=paddle.get_default_dtype(), prefix="deepseek_v3.embed_tokens"
        )
        self.layers = nn.LayerList([DeepSeekV3DecoderLayer(
            fd_config, 
